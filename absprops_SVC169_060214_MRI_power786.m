...
for i=1:size(rightmua)
    ...
    %Calculate musp at 786 using Mie Scattering Power Law: Graff, 1992
    lambda = [826 688 786].*10^-9;
    powerLawB(i) = -log(rightmusp(i,1)/rightmusp(i,2))/log(lambda(1)/lambda(2));
    powerLawA(i) = rightmusp(i,1)/(lambda(1)^(-powerLawB(i)));
    rightmusp(i,3) = powerLawA(i)*lambda(3)^(-powerLawB(i));
...
for i=1:size(leftmua)
    ...
    %Calculate musp at 786 using Mie Scattering Power Law: Graff, 1992
    lambda = [826 688 786].*10^-9;
    powerLawB(i) = -log(leftmusp(i,1)/leftmusp(i,2))/log(lambda(1)/lambda(2));
    powerLawA(i) = leftmusp(i,1)/(lambda(1)^(-powerLawB(i)));
    leftmusp(i,3) = powerLawA(i)*lambda(3)^(-powerLawB(i));
