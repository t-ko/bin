...
%             try
                ...
                [Dbfit_2layer(measnum+1,i,:),fval_2layer(measnum+1,i),exitflag_2layer(measnum+1,i)]=fminsearchbnd(@(x)xg1fit2layer(x,n0,zd,mua2layer(measnum+1,:),musp2layer,lambda*10^7,thickness,r,taustmp,g1tmp),x0,[0 0],[],options);
                %xg1fit2layer_betaandDB_withsigma(x0,n0,R,mua,musp,lambda,ell,rho,taustmp,g2,sigma)
               
                Curvefit2layeravg(:,measnum+1)=g1fit2layer(Dbfit_2layer(measnum+1,i,:),n0,zd,[mua(measnum+1) mua(measnum+1)],[muspo muspo],lambda*10^7,thickness,r,taustmp);
                %Calculate error in fit
