...
%             end
            ...
            Betasaveavg(measnum+1)=median(g2smooth(1:10))-1;
            
            %Semi-inf fit of g2
            [Dbfitavg(measnum+1),fvalavg(measnum+1),exitflagavg(measnum+1)]=fminsearchbnd(@xg2fitx_new1_log,x0(1),[0],[1e-2],options,...
                     Betasaveavg(measnum+1),r,taustmp,muspo,mua(measnum+1),k0,ze,corrstmpavg,length(taustmp),sigma);
...
%             try
                x0 = [Dbfitavg(measnum+1) Dbfitavg(measnum+1)];
                thickness = 0.5;   
                ...
                measnum+1
                [Dbfit_2layer(measnum+1,:),fval_2layer(measnum+1,i),exitflag_2layer(measnum+1,i)]=fminsearchbnd(@(x)xg1fit2layerlog(x,n0,zd,mua2layer(measnum+1,:),musp2layer,lambda*10^7,thickness,r,taustmp(1:datalength),g1tmp(1:datalength,measnum+1)),x0,[0 0],[],options);
                %xg1fit2layer_betaandDB_withsigma(x0,n0,R,mua,musp,lambda,ell,rho,taustmp,g2,sigma)
               
                Curvefit2layeravg(:,measnum+1)=g1fit2layer(Dbfit_2layer(measnum+1,:),n0,zd,mua2layer(measnum+1,:),musp2layer,lambda*10^7,thickness,r,taus);
                hold off
                semilogx(taus(1:datalength),g1avg(1:datalength,measnum+1))
                hold on; axis tight;
                semilogx(taus(1:datalength),Curvefitavg(1:datalength,measnum+1),'black')
                semilogx(taus(1:datalength),Curvefit2layeravg(1:datalength,measnum+1),'red')
                %Calculate error in fit
                ind2layertmp(measnum+1)=min(find(abs(squeeze(Curvefit2layeravg(:,measnum+1))-0.3)==min(abs(squeeze(Curvefit2layeravg(:,measnum+1))-0.3))));%Use min in case size(ind)>1
                errorfit2layeravg(:,measnum+1)=(g1avg(1:length(taus),measnum+1)-squeeze(Curvefit2layeravg(:,measnum+1)))./squeeze(Curvefit2layeravg(:,measnum+1))*100;
                meanerror2layeravg(measnum+1)=mean(errorfitavg(1:indtmp(measnum+1),measnum+1));
                stderror2layeravg(measnum+1)=std(errorfitavg(1:indtmp(measnum+1),measnum+1));
%             catch E
%                 Dbfit_2layer(measnum+1,i,:)=NaN;
%                 fval_2layer(measnum+1,i)=NaN;
%                 exitflag_2layer(measnum+1,i)=NaN;
%                 Curvefit2layeravg(:,measnum+1)=NaN;
%                 ind2layertmp(measnum+1)=NaN;
%                 errorfit2layeravg(:,measnum+1)=NaN;
%                 meanerror2layeravg(measnum+1)=NaN;
%                 stderror2layeravg(measnum+1)=NaN;              
