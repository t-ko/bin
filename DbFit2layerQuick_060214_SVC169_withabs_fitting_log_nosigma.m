...
%             try
                ...
                thickness = 0.5;
                [Dbfit_2layer(measnum+1,:),fval_2layer(measnum+1,i),exitflag_2layer(measnum+1,i)]=fminsearchbnd(@(x)xg1fit2layerlog(x,n0,zd,[mua(measnum+1) mua(measnum+1)],[muspo muspo],lambda*10^7,thickness,r,taustmp(1:datalength),g1tmp(1:datalength)),x0,[0 0],[],options);
                %xg1fit2layer_betaandDB_withsigma(x0,n0,R,mua,musp,lambda,ell,rho,taustmp,g2,sigma)
               
                Curvefit2layeravg(:,measnum+1)=g1fit2layer(Dbfit_2layer(measnum+1,:),n0,zd,[mua(measnum+1) mua(measnum+1)],[muspo muspo],lambda*10^7,thickness,r,taustmp);
                %Calculate error in fit
                ...
                errorfit2layeravg(:,measnum+1)=(g1avg(1:length(taustmp),measnum+1)-squeeze(Curvefit2layeravg(:,measnum+1)))./squeeze(Curvefit2layeravg(:,measnum+1))*100;
