...
cutoff=1.1;%where to cut correlation curve off when fitting
...
if collectedo2
    load([ patID '_' patdate '_dpfout.mat']);
    %Sync ISS and flow 
    if threelambda
        mua=interp1(ISStime,muaseries(3,:),timeaxis_flow)+muao;
    else
        mua=interp1(ISStime,muaseries(2,:),timeaxis_flow)+muao;
    end
end
...
if fitavg==0
    ...
    muao=mua;
    ff=['save ' patID '_' patdate '_MRI_flow_output_fitindiv.mat muao muspo taus Dbfit Curvefit Curvefitg2 corrs g1 intensitydata Marksflow Betasave numframestoavg fval exitflag usedflowdets fitbeta fitavg startcorr datalength avgnum cutoff'];
    eval(ff);
    ...
    muao=mua;
    ff=['save ' patID '_' patdate '_1_flow_output_fitavg.mat timeaxis_flow muao muspo taus Dbfitavg Curvefitavg Curvefitg2avg corrsavg g1avg intensityavg Marksflow numframestoavg fvalavg exitflagavg Betasaveavg usedflowdets fitbeta fitavg startcorr datalength avgnum cutoff'];
    eval(ff);
