...
for n = find(measind)
    n
    %Semi-inf fit of g2
    [Dbfitavg(n),fvalavg(n),exitflagavg(n)]=fminsearchbnd(@(x)xg2fitx_new1_withsigma(x,Betasaveavg(n),r,taustmp,muspo,mua(n),k0,ze,corrstmpavg,length(sigma(n,:)),sigma(n,:)),x0(1),[0],[1e-2],options);
