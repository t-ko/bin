...
parfor measnum=1:(size(intensitydata,1)-1)
    ...
    if fitavg==1
        ...
        intensityavg(measnum)=nanmean(nanmean(intensitydata(measnum:measnum,usedflowdets(1):usedflowdets(2)),2),1);
        corrsavg(measnum,:)=nanmean(nanmean(corrs(measnum:measnum,:,usedflowdets(1):usedflowdets(2)),3),1);
...
%             end
        ...
        for d=1:length(usedflowdets)
            ...
            if isempty(find(isnan(corrs(measnum,:,i)))) & ~isnan(mua(measnum))
                corrsmean=nanmean(corrs(measnum:measnum,:,i),1);
                ...
                g1(:,i,measnum)=sqrt(abs((corrs(measnum:measnum,:,i)-1)./Betasave(measnum,i)));
