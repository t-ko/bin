...
for n = find(measind)
        ...
        n
        %Semi-inf fit of g2
        [Dbfitavg(n),fvalavg(n),exitflagavg(n)]=fminsearchbnd(@(x)xg2fitx_new1_log(x,Betasaveavg(n),r,taustmp,muspo,mua(n),k0,ze,corrstmpavg),x0(1),[0],[1e-2],options);
    ...
    %     endtau = find(abs(log(taus)-endt)==min(abs(log(taus)-endt)));
        ...
        [Dbfit_2layer(n,:),fval_2layer(n),exitflag_2layer(n)]=fminsearchbnd(@(x)xg2fit2layer_log(x,Betasaveavg(n),n0,zd,mua2layer(n,:),musp2layer,lambda*10^7,thickness,r,taustmp(1:endtau),corrstmpavg),x0_2layer,[0 0],[],options);
